...
<x-marketing.page-hero
    title="Blog & Updates"
    subtitle="Stay informed with the latest news, insights, and updates from Prady Technologies"
    :breadcrumbs="[
        ['label' => 'Home', 'url' => '/'],
        ['label' => 'Blog'],
    ]"
...
<!-- Blog Posts -->
    <section class="mkt-section mkt-section--light">
        <div class="mkt-container">
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($posts as $post)
                        <article class="bg-white border-2 border-gray-200 rounded-xl overflow-hidden hover:shadow-xl transition-shadow">
                            <div class="h-48 bg-gradient-to-br from-[var(--prady-ice)] to-[#D6EAF8]" loading="lazy"></div>
                            <div class="p-6">
                                ...
                                @endif
                                <a href="{{ route('blog.show', $post) }}" class="inline-flex items-center text-[var(--prady-blue)] hover:text-[var(--prady-navy)] font-semibold text-sm">
                                    Read more
            ...
            <!-- Newsletter Signup -->
            <div class="mt-16 bg-gradient-to-r from-sky-50 via-white to-[var(--prady-ice)] rounded-xl p-8 text-center">
                ...
                <form id="newsletter-form" action="/newsletter/subscribe" method="POST" class="max-w-md mx-auto flex gap-4">
                    @csrf
                    <input type="email" name="email" placeholder="Enter your email" required class="flex-1 px-4 py-3 rounded-lg text-gray-900 border border-slate-200 focus:ring-2 focus:ring-[var(--prady-cyan)] focus:outline-none">
                    <button type="submit" class="bg-[var(--prady-blue)] text-white px-6 py-3 rounded-lg font-semibold hover:bg-[var(--prady-navy)] transition">
                        Subscribe
                    </button>
                </form>
            </div>
