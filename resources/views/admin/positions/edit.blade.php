    ...
    <div class="max-w-3xl">
        <form action="{{ route('admin.positions.update', $position) }}" method="POST" class="space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="location"
                            class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm md:text-base focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                            required>
                        <option value="Remote" {{ old('location', $position->location) == 'Remote' ? 'selected' : '' }}>Remote</option>
                        <option value="Hybrid" {{ old('location', $position->location) == 'Hybrid' ? 'selected' : '' }}>Hybrid</option>
                        <option value="On-site" {{ old('location', $position->location) == 'On-site' ? 'selected' : '' }}>On-site</option>
                    </select>
                    @error('location')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <input type="text" name="tags" value="{{ old('tags', $position->tags) }}"
                       class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm md:text-base focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $position->order) }}" min="0"
                           class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm md:text-base focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                           placeholder="0">
            ...
            <div class="flex items-center space-x-3 pt-2">
                ...
                <a href="{{ route('admin.positions.index') }}" class="text-sm text-slate-600 hover:underline" data-admin-modal-close data-turbo-frame="admin_main">
                    Cancel
                </a>
            </div>
        </form>
