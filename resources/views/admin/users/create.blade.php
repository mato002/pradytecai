...
@section('page_eyebrow', 'Team access')
@section('page_description', 'Create a user with Spatie role and optional product/social scopes')
...
<option value="{{ $role['slug'] }}" @selected(old('role') === $role['slug'])>{{ $role['label'] }}</option>
@endforeach
</select>
</div>
...
<div>
<p class="text-sm font-semibold mb-2">Product scopes (leave empty = all)</p>
<div class="grid sm:grid-cols-2 gap-2">
@foreach($products as $product)
<label class="flex items-center gap-2 text-sm"><input type="checkbox" name="product_scopes[]" value="{{ $product->id }}" @checked(in_array($product->id, old('product_scopes', [])))> {{ $product->name }}</label>
@endforeach
</div>
</div>
<div>
<p class="text-sm font-semibold mb-2">Social account scopes (leave empty = all)</p>
<div class="grid sm:grid-cols-2 gap-2">
@forelse($socialAccounts as $account)
<label class="flex items-center gap-2 text-sm"><input type="checkbox" name="social_account_scopes[]" value="{{ $account->id }}" @checked(in_array($account->id, old('social_account_scopes', [])))> {{ $account->name }} ({{ $account->platform }})</label>
@empty
